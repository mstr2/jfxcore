...
public class CommandBindingList extends TrackableObservableList<CommandBinding<?>> {
        ...
        while (c.next()) {
            if (c.wasRemoved()) {
                for (CommandBinding<?> binding : c.getRemoved()) {
                    CommandBindingHelper.setNode(binding, null);
                }
            }

            if (c.wasAdded()) {
                for (CommandBinding<?> binding : c.getAddedSubList()) {
                    CommandBindingHelper.setNode(binding, node);
                }
            }
        }
    }
